...
sub cgiapp_prerun {
	...
	my $set_status = sub {
		...
		$tmpl_params->{LASTLOGIN} = UnixDate("epoch " . $app->authen->last_login, "%a, %e %b %y %H:%M:%S");
		$tmpl_params->{THISURL} = $app->query->url;
		$tmpl_params->{ATTEMPTS} = $app->authen->login_attempts();
...
sub cityimages {
	...
	opendir my $imgdir, $app->config_param('default.ImagePath') or die "Cannot Open Directory: $!";
...
sub quotes {
	...
	if (my $username = $app->authen->username) {
		...
		$sth = $app->dbh->prepare("select id, addedon from quotes where username = '$username' order by addedon desc");
		$sth->execute;
